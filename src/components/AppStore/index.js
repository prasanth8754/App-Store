...
class AppStore extends Component {
  ...
  filterApps = () => {
    ...
    return appsList.filter(eachItem => eachItem.category === activeTabId)
  }

  render() {
    const {activeTabId, searchInput} = this.state
    const filteredAppsList = this.filterApps()
    const finalFilteredAppsList = filteredAppsList.filter(eachItem =>
        ...
        <div className="container">
          ...
          <ul className="tabs-container">
            {tabsList.map(eachItem => (
              <TabItem
                tabDetails={eachItem}
                onClickTabItem={this.onChangeTabItem}
